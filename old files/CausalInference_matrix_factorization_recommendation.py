...
# Step 3: 初始化矩陣 P 和 Q
NUM_FACTORS = 10
LEARNING_RATE = 0.001  # Reduced learning rate to avoid gradient explosion
NUM_ITERATIONS = 1000
REGULARIZATION = 1.0  # Increased regularization to stabilize training

P = np.random.rand(num_nodes, NUM_FACTORS) * 0.01  # Smaller initialization to avoid large values
Q = np.random.rand(num_nodes, NUM_FACTORS) * 0.01
...
def predict_relationships(factor_name=None, defect_name=None):
    if factor_name is not None:
        if factor_name in recommendation_df.index:
            print(f"Predicted relationships for factor '{factor_name}':\n", recommendation_df.loc[factor_name])
        else:
            print(f"Factor '{factor_name}' not found in the graph.")
    elif defect_name is not None:
        if defect_name in recommendation_df.columns:
            print(f"Predicted relationships for defect '{defect_name}':\n", recommendation_df[defect_name])
        else:
...
# Step 10: 使用範例
predict_relationships(factor_name=first)  
predict_relationships(defect_name=last)  
