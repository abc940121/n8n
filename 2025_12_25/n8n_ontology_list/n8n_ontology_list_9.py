#add affinity.json, it is same as 8
[
    # ----------------------------
    ...
    "param_placeholder",
    "param_hint",                   # tooltip / hint text (e.g. Adalo collection hint)
    "param_no_data_expression",
    "param_is_node_setting",        # isNodeSetting (e.g. requestOptions)

    # UI / visibility logic
    "param_display_options",        # normalized

    # Options & constraints (standardized)
    "param_type_options",           # normalized dict: {multiple_values, min_value, max_value, rows, button_text, load_options_method, ...}
    ...
    "option_action",
    "option_description",
    "option_routing",               # routing block (request/send/output)
    "option_routing_request",       # HTTP method / url / qs
    "option_routing_send",          # send config (preSend, paginate)
    "option_routing_output",        # output transforms (postReceive)
