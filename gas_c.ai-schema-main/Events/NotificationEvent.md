...
- **只要達到以下條件會執行此 Event 的 Action，判斷之先後順序**
  ...
  3. 訊息類型 (Activity Type) 為 `invoke`
  4. InvokeActivity.Name 與 NotificationName 相符時 (不分大小寫)，且 NotificationName 不得為空白
  5. 通知事件之自訂觸發條件 (**ActionConditions**) 需符合條件
...
| *Type*               | string                       | Y        | Event 類型，值為 `notification`                              | **X**    | 1.0  |
| **NotificationName** | string                       | Y        | 符合的通知名稱，在這裡會依據 Notification Name 去判斷是否觸發這個訊息 | **O**    | 1.0  |
| **ActionConditions** | [EventActionCondition](#-notification-event-action-condition) | N | 通知事件之自訂觸發條件，會在動作 (包含變數操作) 之前判斷，預設值：**為空** (沒有條件限制) | **X** | 1.10 |
...
> **為了避免出現不良的使用者體驗，Event Action 建議只使用的訊息顯示**

### ■ Notification Event Action Condition
...
```json
{
    ...
    "NotificationName": "PushForwardLeave",
    "Action": {
        "Type": "message",
        "Messages": [
            {
...
* **Input Activity  `(Invoke Activity)`**
  * **type** ─ 必須為 `invoke`
  * **name** ─  對應 Schema 所定義的 `NotificationName`
