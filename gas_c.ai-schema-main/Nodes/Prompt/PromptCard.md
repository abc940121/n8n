...
| --------------- | ----------------------------------------------------- | -------- | -------------------------------------------------- | ----------------- | --------------- |
| *Id*            | string                                                | Y        | Node ID                                            | **X** | 1.11   |
| *Name*          | string                                                | N        | Node 名稱                                          | **X** | 1.11   |
| *Description*   | string                                                | N        | Node 描述                                          | **X** | 1.11   |
| *Type*          | string                                                | Y        | Node 類型，值為 `prompt.card`              | **X** | 1.11   |
| **Prompt**      | [MessageContent](../../Contents/MessageContent.md)    | Y        | 提示使用者需要的內容，例如：卡片                   | **X** | 1.11   |
| **Retry**       | [MessageContent](../../Contents/MessageContent.md)    | N        | 提示使用者輸入錯誤，未設定時沿用 **Prompt** 的設定 | **X** | 1.11   |
| **RetryMaxCount** | iny | N | 重新輸入最大次數，預設為 `-1` (沒有限制次數)  **`<計畫中>`** | **X** | ?? |
| **Validator**   | [RuleConditions](BasePrompt.md#-prompt-validator) | N        | 驗證使用者輸入的格式                          | **X** | 1.11 |
| *Actions*       | [NodeAction[]](../../Actions/NodeAction.md)           | N        | Node 轉換行為             | **X** | 1.11   |
| *VariableActions* | [VariableAction[]](../../Variables/VariableAction.md) | N | 處理自訂變數 | **X** | 1.11 |
...
* **限制說明：**
    * 在發送提問訊息的同時會在 ChannelData 中放入限制輸入的資訊
        * Web Chat 可以透過限制輸入的資訊，在 Client 端限制使用者輸入
    * 僅限以下訊息類型會放入限制輸入的資訊
        * **HeroCard**
        * **Signin Card**
        * **Animation Card**
        ...
        * **Adaptive Card** (含衍生類型)
        * **Quick Reply**
* **Channel Data 之限制輸入的資訊**
    * 會在 Activity.ChannelData 中，放入限制輸入的資訊

```json
{
    "ChannelData": {
        "event": {
            "type": "DisableTextMessageInput",
            "payload": {
                "isDisable": true
            }
        }
